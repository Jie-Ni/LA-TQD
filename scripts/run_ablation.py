    ...
    for run_idx in range(n_runs):
        ...
        for seq in seeds:
            ...
            if (check_length(seq) and check_alphabet(seq) and check_rgd_motif(seq)
                    and (not use_poly or max_single_aa_count(seq) <= MAX_AA_FREQ)
                    and (not use_aromatic or aromatic_count(seq) >= MIN_AROMATIC)):
                score = fitness(seq, esm_model, esm_tokenizer)
                if score != float("inf"):
        ...
        for rnd in range(1, n_rounds + 1):
            ...
            if not (check_length(child) and check_alphabet(child) and check_rgd_motif(child)):
                continue
            if use_poly and max_single_aa_count(child) > MAX_AA_FREQ:
                continue
            if use_aromatic and aromatic_count(child) < MIN_AROMATIC:
                continue
            score = fitness(child, esm_model, esm_tokenizer)
            if score != float("inf"):
